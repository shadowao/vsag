        ...
        for (int i = 0; i < dim; i += next_step) {
            ...
            if(step>8 && step % 8 == 0){
                for (int j = 0; j < step; j+=16) {
                    __m512 g1 = _mm512_loadu_ps(&vec[i + j]);
                    __m512 g2 = _mm512_loadu_ps(&vec[i + j + step]);
                    _mm512_storeu_ps(&vec[i + j], _mm512_add_ps(g1, g2));
                    _mm512_storeu_ps(&vec[i + j + step], _mm512_sub_ps(g1, g2));
                }
            } else {
