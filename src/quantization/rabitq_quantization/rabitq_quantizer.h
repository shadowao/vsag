...
public:
    ...
    void
    EncodeSQ(
        const DataType* normed_data, uint8_t* encode_data, float &upper_bound, float &lower_bound, float &delta, sum_type &query_sum) const;
    void
    DecodeSQ(
        const uint8_t* codes, DataType* data, const float upper_bound, const float lower_bound) const;
...
RaBitQuantizer<metric>::ComputeQueryBaseImpl(const uint8_t* query_codes,
    ...
    if (num_bits_per_dim_query_ == 4) {
        Vector<uint8_t> tmp(aligned_dim_ / 8, 0, this->allocator_);
...
template <MetricType metric>
void
RaBitQuantizer<metric>::EncodeSQ(
    const DataType* normed_data, uint8_t* quantized_data, float &upper_bound, float &lower_bound, float &delta, sum_type &query_sum) const
{
    ...
    const float inv_delta =
        (std::abs(delta) > std::numeric_limits<float>::epsilon()) ? 1.0f / delta : 0.0f;
    query_sum = 0;
...
template <MetricType metric>
void
RaBitQuantizer<metric>::DecodeSQ(
    const uint8_t* codes, DataType* data, const float upper_bound, const float lower_bound) const
{
    for (uint64_t d = 0; d < this->dim_; d++) {
        data[d] = codes[d] / (float)((1 << num_bits_per_dim_query_) - 1) * (upper_bound - lower_bound) + lower_bound;
    }
}
...
RaBitQuantizer<metric>::ProcessQueryImpl(const DataType* query,
    ...
    try {
        ...
        if (num_bits_per_dim_query_ == 4) {
            ...
            EncodeSQ(normed_data.data(), quantized_data.data(), upper_bound, lower_bound, delta, query_sum);
